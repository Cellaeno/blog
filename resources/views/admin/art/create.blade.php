            ...
            <div class="main-page">
                <div class="forms">
                    ...
                    <div class="form-grids row widget-shadow" data-example-id="basic-forms"> 
                        ...
                        <div class="form-body">
                            <form action="/admin/art/store" method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                <div class="form-group">
                                    <label for="title">标题</label>
                                    <input type="text" class="form-control" name="title" value="{{old('title')}}" id="title" placeholder="标题">
                                </div>
                                <div class="form-group">
                                    <label for="auth">作者</label>
                                    <input type="text" class="form-control" value="{{old('auth')}}" name="auth" id="auth">
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="cid" class="form-control">
                                        ...
                                        @foreach($data_cates as $data_cate)
                                            @if($data_cate->pid == 0)
                                            <option value="{{$data_cate->cid}}" style="font-weight: 800;">{{$data_cate->cname}}</option>
                                            @else
                                            <option value="{{$data_cate->cid}}">{{$data_cate->cname}}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="tid" class="form-control">
                                        ...
                                        @foreach($data_tags as $data_tag)
                                            <option value="{{$data_tag->tid}}">{{$data_tag->tname}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
